...
import module.model_efmp as model_efmp
...
# --------------------------------------------------------
def get_args_parser():
    parser = argparse.ArgumentParser('efmp pre-training', add_help=False)
    parser.add_argument('--description', type=str, default='efmp_pretrain')
    ...
    # Model parameters
    parser.add_argument('--model', default='efmp', type=str, metavar='MODEL',
                        help='Name of model to train')

    parser.add_argument('--input_size', default=224, type=int,
                        help='images input size (fixed to 224 after removing SR)')  # 明确说明
...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            mim_loss, mlm_loss = model(batch)  # 移除 res_loss

            loss_value1 = mim_loss.item()
            loss_value3 = mlm_loss.item()
            
            loss = mim_loss + mlm_loss
            ...
            metric_logger.update(mlm_loss=loss_value3)  # 移除 res_loss 记录
            ...
            if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
                ...
                log_writer.add_scalar('mlm_loss', loss_value_reduce3, epoch_1000x)  # 移除 res_loss 的日志记录
...
def main(args):
    ...
    # define the model
    model = model_efmp.__dict__[args.model](norm_pix_loss=args.norm_pix_loss)
