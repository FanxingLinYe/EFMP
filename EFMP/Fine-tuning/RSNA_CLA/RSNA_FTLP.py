...
def setup(args):
    args.is_multilabel = False  # RSNA is single-label binary classification
...
def test(args):
    ...
    one_hot_labels = np.zeros_like(all_preds)
    for i, label in enumerate(all_label):
        one_hot_labels[i, label] = 1
    
    # 计算AUC
    aurocs = auc(all_preds, one_hot_labels, args.num_classes)
    
    logger.info("\n")
    logger.info("Test Results")
    logger.info(f"Test Loss: {eval_losses.avg:.5f}")
    for i, auroc in enumerate(aurocs):
        logger.info(f"Test AUC for class {RSNA_CLASS_NAMES[i] if i < len(RSNA_CLASS_NAMES) else i}: {auroc:.5f}")
    logger.info(f"Mean AUC: {np.mean(aurocs):.5f}")
...
def main():
    ...
    parser.add_argument("--num_classes", default=2, type=int, help="Number of classes for RSNA")
    ...
    parser.add_argument("--dataset_path", type=str, help="Path to RSNA dataset.")
