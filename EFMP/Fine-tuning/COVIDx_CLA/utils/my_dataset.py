...
class XRAY(data.Dataset):
    ...
    def __init__(self, root, data_volume, split="train", transform=None):
        super(XRAY, self)
        if data_volume == '1':
            train_label_data = "filtered_train_list_1.txt"
        if data_volume == '10':
            train_label_data = "filtered_train_list_10.txt"
        if data_volume == '100':
            train_label_data = "filtered_train_list.txt"
        test_label_data = "filtered_test_list.txt"
        val_label_data = "filtered_val_list.txt"
        ...
        # root_tmp = os.path.join(root,self.split,"images")
        root_tmp = os.path.join(root,"")
        while line:
