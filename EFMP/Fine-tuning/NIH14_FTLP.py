...
NIH_CLASS_NAMES = ['Atelectasis', 'Cardiomegaly', 'Effusion', 'Infiltration', 'Mass', 'Nodule', 'Pneumonia','Pneumothorax', 'Consolidation', 'Edema', 'Emphysema', 'Fibrosis', 'Pleural_Thickening', 'Hernia']
...
def setup(args):
    # ChestX-ray14 is a multilabel task
...
def valid(args, model, writer, test_loader, global_step):
    ...
    model.eval()
    all_preds, all_label, all_property = [], [], []
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.no_grad():
            ...
            if len(all_preds) == 0:
                ...
                all_property[0] = np.append(all_property[0], logits.sigmoid().detach().cpu().numpy(), axis=0)
...
def main():
    ...
    parser.add_argument("--task", choices=["ChestX-ray14"], default="ChestX-ray14", help="Finetune task.")
    ...
    parser.add_argument("--num_steps", default=10000, type=int, help="Total number of training steps to perform.")
